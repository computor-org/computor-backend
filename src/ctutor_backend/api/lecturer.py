...
async def lecturer_get_courses(course_id: UUID | str, permissions: Annotated[Principal, Depends(get_current_principal)], db: Session = Depends(get_db)):
...
def lecturer_list_courses(permissions: Annotated[Principal, Depends(get_current_principal)], params: CourseQuery = Depends(), db: Session = Depends(get_db)):
...
def lecturer_get_course_contents(course_content_id: UUID | str, permissions: Annotated[Principal, Depends(get_current_principal)], db: Session = Depends(get_db)):
...
def lecturer_list_course_contents(permissions: Annotated[Principal, Depends(get_current_principal)], params: CourseContentLecturerQuery = Depends(), db: Session = Depends(get_db)):

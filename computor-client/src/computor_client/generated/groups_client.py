...
class GroupsClient(BaseEndpointClient):
    ...
    async def post_groups(self, payload: GroupCreate, user_id: Optional[str] = None) -> GroupGet:
        """Create Groups"""
    ...
    async def get_groups(self, skip: Optional[str] = None, limit: Optional[str] = None, id: Optional[str] = None, name: Optional[str] = None, group_type: Optional[str] = None, archived: Optional[str] = None, user_id: Optional[str] = None) -> List[GroupList]:
        """List Groups"""
        params = {k: v for k, v in locals().items() if k in ['skip', 'limit', 'id', 'name', 'group_type', 'archived', 'user_id'] and v is not None}
        data = await self._request("GET", "", params=params)
        if isinstance(data, list):
            return [GroupList.model_validate(item) for item in data]
        return data

    async def get_group_by_id(self, id: str, user_id: Optional[str] = None) -> GroupGet:
        """Get Groups"""
    ...
    async def patch_group_by_id(self, id: str, payload: GroupUpdate, user_id: Optional[str] = None) -> GroupGet:
        """Update Groups"""
        json_data = payload.model_dump(mode="json", exclude_unset=True) if hasattr(payload, "model_dump") else payload
    ...
    async def delete_group_by_id(self, id: str, user_id: Optional[str] = None) -> Any:
        """Delete Groups"""

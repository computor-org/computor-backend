...
class LanguagesClient(BaseEndpointClient):
    ...
    async def get_language_by_id(self, id: str) -> LanguageGet:
        """Get Languages"""
    ...
    async def get_languages(self, skip: Optional[str] = None, limit: Optional[str] = None, code: Optional[str] = None, name: Optional[str] = None, native_name: Optional[str] = None) -> List[LanguageList]:
        """List Languages"""
        params = {k: v for k, v in locals().items() if k in ['skip', 'limit', 'code', 'name', 'native_name'] and v is not None}

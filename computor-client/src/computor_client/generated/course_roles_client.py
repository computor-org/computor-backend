...
class CourseRolesClient(BaseEndpointClient):
    ...
    async def get_course_role_by_id(self, id: str) -> CourseRoleGet:
        """Get Course-Roles"""
    ...
    async def get_course_roles(self, skip: Optional[str] = None, limit: Optional[str] = None, id: Optional[str] = None, title: Optional[str] = None, description: Optional[str] = None) -> List[CourseRoleList]:
        """List Course-Roles"""
        params = {k: v for k, v in locals().items() if k in ['skip', 'limit', 'id', 'title', 'description'] and v is not None}

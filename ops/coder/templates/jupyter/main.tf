# JupyterLab workspace. See README.md for the template contract.
...
locals {
  ...
  # /home/coder, so files and user-space installs (pip install --user) follow
  # the user across workspaces. The volume is deliberately NOT a Terraform
  # resource — the docker engine auto-creates it on first mount, and because
  # Terraform never owns it, deleting a workspace can never destroy the user's
  # home. System (apt/pip) packages live in the image and are NOT persisted.

...
variable "jupyter_port" {
  default     = 8888
  description = "Port for the JupyterLab server"
  type        = number
}

variable "workspace_image" {
  default     = "localhost:5000/computor-workspace-jupyter:latest"
...
variable "coder_base_path" {
  default     = "/coder"
  description = "Base path prefix for workspace access via Traefik. JupyterLab serves under this full prefix (base_url) — this template does NOT strip it."
  type        = string
}

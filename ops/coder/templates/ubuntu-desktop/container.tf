...
resource "docker_container" "workspace" {
  ...
  # Traefik labels for desktop access at /coder/{username}/{workspace}
  labels {
  ...
  # Injects the per-user app credential (KasmVNC checks HTTP basic auth against ~/.kasmpasswd), so a request reaching the app
  ...
  labels {
    ...
    value = local.app_secret != "" ? "Basic ${base64encode("coder:${local.app_secret}")}" : ""
  }

...
def validate_file_extension(filename: str) -> Tuple[bool, Optional[str]]:
    ...
    # Allow files without extensions (common for data files, configs, etc.)
    if not ext:
        return True, None
...
def validate_content_type(content_type: str, filename: str) -> Tuple[bool, Optional[str]]:
    """
    Validate MIME type against blacklist.
    ...
    if content_type in BLOCKED_MIME_TYPES:
        return False, f"Content type '{content_type}' is not allowed for security reasons"
...
def check_file_content_security(file_data: BinaryIO, filename: str) -> Tuple[bool, Optional[str]]:
    ...
    if header.startswith(b'#!'):
        ext = os.path.splitext(filename)[1].lower()
        # Allow shebangs in recognized script files
        allowed_shebang_extensions = {'.sh', '.py', '.pl', '.rb', '.ps1', '.bat', '.bash', '.zsh', '.fish'}
        if ext and ext not in allowed_shebang_extensions:
...
def perform_full_file_validation(
    ...
    # Validate file extension (blacklist check)
    valid, error = validate_file_extension(filename)
    if not valid:
        raise BadRequestException(detail=error)

    # Validate content type (blacklist check)
    ...
    logger.debug(f"File validation passed for: {filename} ({format_bytes(file_size)})")

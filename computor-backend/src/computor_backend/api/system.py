...
async def sync_documents_repository(
    ...
    # Check permissions - user must be lecturer in any course of this family or admin
    if not check_admin(permissions):
        from ..model.course import Course
        from ..model.user_course import UserCourse

        # Check if user is lecturer in any course of this course family
        is_lecturer = db.query(UserCourse).join(
            Course, Course.id == UserCourse.course_id
        ).filter(
            Course.course_family_id == course_family_id,
            UserCourse.user_id == permissions.user_id,
            UserCourse.role_id == "_lecturer"
        ).first()

        if not is_lecturer:
            raise ForbiddenException(
                detail="Only lecturers or administrators can sync documents",
                context={"course_family_id": course_family_id}
            )

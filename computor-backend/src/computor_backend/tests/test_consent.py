...
class TestConsentGateMiddleware:
    ...
    def test_unconsented_user_blocked_with_stable_body(self, client, fake_redis):
        ...
        body = response.json()
        # The gate now carries a machine-readable code plus a human message so
        # clients can surface it directly (see the consent-error surfacing work);
        # the original two keys stay part of the contract.

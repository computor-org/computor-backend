...
from computor_types.organizations import OrganizationGet, OrganizationType
from computor_types.profiles import ProfileGet
from computor_types.sessions import SessionGet, SessionList
from computor_types.user_groups import UserGroupGet
from computor_types.group_claims import GroupClaimGet
...
class TestComputedProperties:
    ...
    def test_session_activity_properties(self):
        ...
        assert "Firefox on Linux" in active_session.display_name
        assert "Active" in active_session.display_name

        # Ended session (ended_at drives is_active; session_duration uses it too)
        ended_session = SessionGet(
            id="456",
            sid="sid-ended",
            user_id="user-123",
            session_id="session-ended-123456789",
            created_ip="192.168.1.2",
            device_label="Safari on macOS",
            created_at=base_time,
            ended_at=base_time + timedelta(hours=2, minutes=30)
        )
        assert ended_session.is_active is False
        assert ended_session.session_duration == 9000  # 2.5 hours in seconds
        assert "Ended" in ended_session.display_name

    def test_session_list_properties(self):
        """Test SessionList properties"""
        ended_session_list = SessionList(
            id="123",
            sid="sid-123",
            user_id="user-123",
        ...
        assert ended_session_list.is_active is False
        assert "10.0.0.1" in ended_session_list.display_name
        assert "Ended" in ended_session_list.display_name

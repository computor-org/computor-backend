...
async def test_submit_issue_report_creates_issue_and_redacts_context():
    ...
    configuration = {
        ...
        "GITHUB_ISSUE_REPORT_LABELS": "Testing, Usability",
    ...
    with patch.multiple(issue_reports.settings, **configuration), patch.object(
        issue_reports.httpx,
        "AsyncClient",
...
def test_issue_report_requires_explicit_server_configuration():
    with patch.multiple(
        issue_reports.settings,
        GITHUB_ISSUE_REPORT_ENABLED=False,
        GITHUB_ISSUE_REPORT_TOKEN=None,
    ), pytest.raises(issue_reports.IssueReportNotConfigured):

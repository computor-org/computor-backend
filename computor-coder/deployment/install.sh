...
# CODER_PROTOCOL defaults to https unless -H flag is used
...
if [ -z "$CODER_DOMAIN" ]; then
  read -p "Enter domain (e.g. coder.example.com): " CODER_DOMAIN
fi
...
CODER_ACCESS_URL="${CODER_PROTOCOL}://${CODER_DOMAIN}:${CODER_PORT}"
...
# Coder deployment configuration (generated by install.sh)
CODER_DIR=${CODER_DIR}
CODER_DOMAIN=${CODER_DOMAIN}
CODER_PORT=${CODER_PORT}
CODER_PROTOCOL=${CODER_PROTOCOL}
CODER_POSTGRES_PORT=${CODER_POSTGRES_PORT}
CODER_POSTGRES_PASSWORD=${CODER_POSTGRES_PASSWORD}
DOCKER_GID=${DOCKER_GID}
CODER_ADMIN_USERNAME=${CODER_ADMIN_USERNAME}
...
if [ "$HAVE_PYTHON_TEMPLATE" = true ]; then
  echo "  - python-workspace (Python 3.13 + code-server)"
fi
...
echo "  ./create-user.sh -t python-workspace ..."

...
export default function CourseFamilyEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiFetch(`${API_BASE_URL}/course-families/${familyId}`);
        if (!res.ok) throw new Error('Failed to load course family');
        const f: CourseFamilyGet = await res.json();
        if (cancelled) return;
        setFamily(f);
        setTitle(f.title || '');
  ...
  if (!authLoading && isAuthenticated && !canManage) {
    return (
      <AuthenticatedLayout>
        <NotFound title="Not available" message="You do not have permission to edit course families." />
      </AuthenticatedLayout>
    );
  }

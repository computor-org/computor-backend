...
| 408 | Adopt or delete UI primitives (Button, useNotify, tables) | P2 | M | done (merged, PARTIAL; src/components/ui/ Button+Table(Th/Td…)+tokens(inputCls, 20 importers repointed); 31/31 table headers swept byte-identically. Buttons NOT swept (all 23 raw variants differ from the primitive's class set — a deliberate restyle is a separate non-no-op task). useNotify KEPT — review was stale, it has 9 live consumers. tsc+build clean.) |
